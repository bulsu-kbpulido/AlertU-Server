...
transporter.verify((error) => {
  if (error) {
    console.error('❌ Admin Password Reset SMTP Verification Error:', error.message);
  } else {
    console.log('✅ Admin Password Reset SMTP Transporter ready');
  }
});

// 1. Send Admin Password Reset 6-Digit OTP
router.post('/send-admin-reset-otp', async (req, res) => {
  ...
  try {
    ...
    // 💡 Verify target account exists in 'admins' Firestore collection
    let isAdmin = false;
    ...
    if (!isAdmin) {
      return res.status(403).json({ success: false, error: 'Access denied: Email address does not belong to an Admin account.' });
    }
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e0e0e0; border-radius: 12px; background-color: #ffffff;">
          <h2 style="color: #0d47a1; text-align: center; margin-bottom: 8px;">Admin Password Reset</h2>
          <p style="color: #4a5568; font-size: 14px; text-align: center;">You requested to reset your Admin account password. Use the verification code below to proceed:</p>
...
router.post('/reset-admin-password', async (req, res) => {
  ...
  try {
    ...
    console.error('Admin password reset update error:', error);
