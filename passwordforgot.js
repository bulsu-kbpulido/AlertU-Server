...
// 🔑 Import modular getAuth from firebase-admin/auth
const { getAuth } = require('firebase-admin/auth');

const otpStore = new Map();

// 📧 Use explicit host, port 465, and secure: true (matches EmailVerificationRoute)
...
transporter.verify((error) => {
  if (error) {
    console.error('❌ Password Reset SMTP Verification Error:', error.message);
  } else {
    console.log('✅ Password Reset SMTP Transporter ready');
  }
});
...
router.post('/send-reset-otp', async (req, res) => {
  ...
  try {
    // 💡 Replaced admin.auth().getUserByEmail() with getAuth().getUserByEmail()
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; border: 1px solid #e0e0e0; border-radius: 12px; background-color: #ffffff;">
          <h2 style="color: #0d47a1; text-align: center; margin-bottom: 8px;">AlertU Password Reset</h2>
          <p style="color: #4a5568; font-size: 14px; text-align: center;">You requested to reset your password. Use the verification code below in the app:</p>
          ...
          <p style="color: #718096; font-size: 12px; text-align: center; margin: 0;">This code expires in 10 minutes. If you did not request this, please ignore this email.</p>
        </div>
      `,
    };
...
router.post('/reset-password', async (req, res) => {
  ...
  try {
    // 💡 Replaced admin.auth().updateUser() with getAuth().updateUser()
